    ...
    @usuario_matricula = usuario_matricula
      ...
      @relatorio.each do |relatorio|
        if(relatorio.matricula_aluno == @usuario_matricula)
          ...
          text "inscrita no CNPJ/MF sob o Nº <u>#{relatorio.CNPJ_estagio}</u>, situada na Rua <u>#{relatorio.rua_estagio}</u>, Nº <u>#{relatorio.numero_estagio}</u>, bairro <u>#{relatorio.bairro_estagio}</u>, município <u>#{relatorio.municipio_estagio}</u>, UF <u>#{relatorio.estado_estagio}</u>, CEP <u>#{relatorio.cep_estagio}</u>, telefone <u>#{relatorio.telefone_estagio}</u>, representada por <u>#{relatorio.representado_por}</u>, para fins de abertura de campo de Estágio ao <b>LICENCIANDO</b>, <u>#{relatorio.nome}</u>, matriculado sob o nº <u>#{relatorio.matricula_aluno}</u>, no <u>#{relatorio.periodo}</u> período do ano letivo de <u>#{relatorio.ano}</u>, no <u>#{relatorio.semestre}</u> semestre, no Curso Superior de Licenciatura em <u>#{relatorio.licenciatura}</u>, residente na Rua <u>#{relatorio.endereco}</u>, Nº <u>#{relatorio.numero}</u>, bairro <u>#{relatorio.bairro}</u>, município <u>#{relatorio.municipio}</u>, Estado do <u>#{relatorio.estado}</u>, CEP.: <u>#{relatorio.CEP}</u>, nos termos da Lei Nº 11.788 de 25/09/2008, cujas cláusulas constam e regem o presente contrato:", align: :justify, :inline_format => true, :leading => 10
          move_down 30
