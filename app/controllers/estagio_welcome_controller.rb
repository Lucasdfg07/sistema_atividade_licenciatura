class EstagioWelcomeController < ApplicationController
    def index
      @users = User.order(nome: :ASC).where("situacao != '0' AND (nome LIKE :search OR matricula LIKE :search OR licenciatura LIKE :search OR periodo LIKE :search OR email LIKE :search)", search: "%#{params[:search]}%").paginate(page: params[:page], per_page: 10)
      if @users.exists?
      else
        @users = User.order(nome: :ASC).where("situacao != '0' AND (nome LIKE :search OR matricula LIKE :search OR licenciatura LIKE :search OR periodo LIKE :search OR email LIKE :search)", search: "%#{params[:search]}%".titleize).paginate(page: params[:page], per_page: 10)
        if @users.exists?
        else
          @users = User.order(nome: :ASC).where("situacao != '0' AND (nome LIKE :search OR matricula LIKE :search OR licenciatura LIKE :search OR periodo LIKE :search OR email LIKE :search)", search: "%#{params[:search]}%".titleize).paginate(page: params[:page], per_page: 10)
        end
      ...
      @will_paginate = User.where("situacao != '0'").order(nome: :ASC).paginate(page: params[:page], per_page: 10)
    end
